return {
	...
	config = function()
		...
		basic.arrow = {
			...
			hl_colors = hl_list.default,
			text = function()
				-- Safely check if arrow is available
				local ok, arrow_statusline = pcall(require, "arrow.statusline")
				if ok then
					local text = arrow_statusline.text_for_statusline_with_icons()
					-- Only return the table if text isn't empty
					if text and text ~= "" then
						return {
							{ " " .. text .. " ", hl_list.default },
						}
					end
				end
				return ""
